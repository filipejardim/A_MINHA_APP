...
static final List<dynamic> earlyCandidates = [];
  ...
  static String uiLanguage = 'EN'; // idioma escolhido (para notificações/CallKit fora da app)
  static DateTime? callLastActive; // última vez que houve uma chamada ativa
...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  // Este código corre num isolate à parte, sem o idioma em memória: lê-o das preferências.
  PadlockNetwork.uiLanguage = (await SharedPreferences.getInstance()).getString('app_language') ?? 'EN';
  ...
  if (isCall) {
    ...
    await FlutterCallkitIncoming.showCallkitIncoming(
      CallKitParams(
        ...
        appName: 'Padlock',
        avatar: '',
        handle: (t[PadlockNetwork.uiLanguage] ?? t['EN']!)['call_handle_text']!,
        type: 0,
        duration: 60000,
        textAccept: (t[PadlockNetwork.uiLanguage] ?? t['EN']!)['call_accept_text']!,
        textDecline: (t[PadlockNetwork.uiLanguage] ?? t['EN']!)['call_decline_text']!,
    ...
    await localNotif.show(
      notificationId,
      'Padlock',
      (t[PadlockNetwork.uiLanguage] ?? t['EN']!)['notif_body_message']!,
      ...
      FlutterCallkitIncoming.endAllCalls();
        ...
        if (Hive.isBoxOpen('padlock_vault')) {
          ...
          if (idx != -1) {
            ...
            allChats[idx]['messages'].add({'text': '📞 ${(t[PadlockNetwork.uiLanguage] ?? t['EN']!)['missed_secure_call']} ($timeStr)', 'isMe': false, 'status': 'missed', 'timestamp': DateTime.now().millisecondsSinceEpoch});
            allChats[idx]['msg'] = '📞 ${(t[PadlockNetwork.uiLanguage] ?? t['EN']!)['missed_secure_call']}';
...
androidImplementation?.requestNotificationsPermission();
  ...
  PadlockNetwork.uiLanguage = savedLanguage;
...
class _PadlockAppState extends State<PadlockApp> with WidgetsBindingObserver {
  ...
  void _changeLanguage(String lang) {
    PadlockNetwork.uiLanguage = lang;
    setState(() {
...
Map<String, Map<String, String>> t = {
  'EN': {
    ...
    'premium_required_body': 'Store all your cryptocurrencies in a military-grade local vault. Send funds to anyone and receive from anywhere, with zero middlemen and absolute privacy.\n\nUnlocking the Secure Crypto Vault requires a Premium subscription.',
    'monthly_button': 'MONTHLY',
    'yearly_button': 'YEARLY',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'waiting...',
    'security_alert_key_changed': 'SECURITY ALERT: Key changed. Connection blocked!',
    'default_username': 'User',
    'node_destroyed': 'Node destroyed',
    'call_handle_text': 'Encrypted Call',
    'call_accept_text': 'Accept',
    'call_decline_text': 'Decline',
    'notif_title_new_message': 'New Message',
    'notif_body_message': 'You have received an encrypted message.',
    'notif_body_file': 'You have received an encrypted file.',
  },
  'PT': {
    'chats': 'Conversas',
    'contacts': 'Contactos',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Cofre Cripto Seguro',
    ...
    'help_vault_files_q': 'Como usar os Ficheiros do Cofre Seguro?',
    ...
    'secure_crypto_vault_short': 'Cofre Cripto\nSeguro',
    'secure_vault_files_short': 'Ficheiros do\nCofre Seguro',
    ...
    'crypto_vault_not_initialized': 'Cofre Cripto não inicializado neste dispositivo.',
    'invalid_crypto_vault_code': 'Código do Cofre Cripto inválido.',
    'crypto_vault_corrupted': 'Os dados do Cofre Cripto estão corrompidos (o código estava certo, mas o próprio ficheiro do cofre está danificado).',
    'create_crypto_vault_code_title': 'CRIAR CÓDIGO DO COFRE CRIPTO',
    'enter_crypto_vault_code_title': 'INTRODUZIR CÓDIGO DO COFRE CRIPTO',
    'crypto_code_desc_create': 'Este código é separado dos códigos da app e dos Ficheiros do Cofre. Protege uma carteira nova, não-custodial, que só tu controlas.',
    'crypto_code_desc_enter': 'Introduz o teu código do Cofre Cripto para aceder à tua carteira.',
    ...
    'set_crypto_vault_code_label': 'Definir Código do Cofre Cripto (para ESTE dispositivo)',
    'crypto_vault_code_label': 'Código do Cofre Cripto',
    ...
    'vault_files_not_initialized': 'Os Ficheiros do Cofre Seguro não estão inicializados neste dispositivo.',
    'invalid_vault_files_code': 'Código dos Ficheiros do Cofre Seguro inválido.',
    'vault_files_corrupted': 'Os dados dos Ficheiros do Cofre Seguro estão corrompidos (o código estava correto, mas o próprio ficheiro do cofre está danificado).',
    'create_vault_files_code_title': 'CRIAR CÓDIGO DO FICHEIROS DO COFRE',
    'enter_vault_files_code_title': 'INTRODUZIR CÓDIGO DO FICHEIROS DO COFRE',
    ...
    'vault_files_code_desc_enter': 'Introduz o teu código dos Ficheiros do Cofre para veres as tuas fotos e documentos encriptados.',
    'set_vault_files_code_label': 'Definir Código dos Ficheiros do Cofre',
    'vault_files_code_label': 'Código dos Ficheiros do Cofre',
    ...
    'document_stored_encrypted_desc': 'Este documento está guardado encriptado nos teus Ficheiros do Cofre ({kb} KB). Usa Exportar para o guardares de novo no telemóvel ou partilhares.',
    ...
    'encrypted_photo_sent_message': '🖼️ Foto encriptada enviada — vê nos Ficheiros do Cofre Seguro',
    ...
    'encrypted_file_sent_message': '📎 Ficheiro encriptado enviado — vê nos Ficheiros do Cofre Seguro',
    'file_chat_preview': '📎 Ficheiro',
    'encrypted_file_received': 'Ficheiro encriptado recebido — abre os Ficheiros do Cofre Seguro',
    'premium_required_title': 'PREMIUM NECESSÁRIO',
    'premium_required_body': 'Guarda todas as tuas criptomoedas num cofre local de nível militar. Envia fundos a qualquer pessoa e recebe de qualquer lugar, sem intermediários e com privacidade absoluta.\n\nDesbloquear o Cofre Cripto Seguro exige uma subscrição Premium.',
    'monthly_button': 'MENSAL',
    'yearly_button': 'ANUAL',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'a aguardar...',
    'security_alert_key_changed': 'ALERTA DE SEGURANÇA: Chave alterada. Ligação bloqueada!',
    'default_username': 'Utilizador',
    'node_destroyed': 'Nó destruído',
    'call_handle_text': 'Chamada Encriptada',
    'call_accept_text': 'Atender',
    'call_decline_text': 'Recusar',
    'notif_title_new_message': 'Nova Mensagem',
    'notif_body_message': 'Recebeste uma mensagem encriptada.',
    'notif_body_file': 'Recebeste um ficheiro encriptado.',
  },
  'ES': {
    'chats': 'Chats',
    'contacts': 'Contactos',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Bóveda Cripto Segura',
    ...
    'help_vault_files_q': '¿Cómo usar Archivos de la Bóveda Segura?',
    ...
    'qr_code_button': 'Código QR',
    'copy_id_button': 'Copiar ID',
    'secure_crypto_vault_short': 'Bóveda Cripto\nSegura',
    'secure_vault_files_short': 'Archivos de la\nBóveda Segura',
    ...
    'crypto_vault_not_initialized': 'Bóveda Cripto no inicializado en este dispositivo.',
    'invalid_crypto_vault_code': 'Código de Bóveda Cripto inválido.',
    'crypto_vault_corrupted': 'Los datos del Bóveda Cripto están corruptos (el código era correcto, pero el propio archivo de la bóveda está dañado).',
    'create_crypto_vault_code_title': 'CREAR CÓDIGO DE BÓVEDA CRIPTO',
    'enter_crypto_vault_code_title': 'INTRODUCIR CÓDIGO DE BÓVEDA CRIPTO',
    'crypto_code_desc_create': 'Este código es independiente de los códigos de la app y de Archivos de la Bóveda. Protege una billetera nueva, no custodial, que solo tú controlas.',
    'crypto_code_desc_enter': 'Introduce tu código de Bóveda Cripto para acceder a tu billetera.',
    ...
    'set_crypto_vault_code_label': 'Establecer Código de Bóveda Cripto (para ESTE dispositivo)',
    'crypto_vault_code_label': 'Código de Bóveda Cripto',
    ...
    'vault_files_not_initialized': 'Archivos de la Bóveda no está inicializado en este dispositivo.',
    'invalid_vault_files_code': 'Código de Archivos de la Bóveda inválido.',
    'vault_files_corrupted': 'Los datos de Archivos de la Bóveda están dañados (el código era correcto, pero el propio archivo de la caja fuerte está dañado).',
    'create_vault_files_code_title': 'CREAR CÓDIGO DE ARCHIVOS DE LA BÓVEDA',
    'enter_vault_files_code_title': 'INTRODUCIR CÓDIGO DE ARCHIVOS DE LA BÓVEDA',
    ...
    'vault_files_code_desc_enter': 'Introduce tu código de Archivos de la Bóveda para ver tus fotos y documentos cifrados.',
    'set_vault_files_code_label': 'Establecer Código de Archivos de la Bóveda',
    'vault_files_code_label': 'Código de Archivos de la Bóveda',
    ...
    'document_label': 'Documento',
    'document_stored_encrypted_desc': 'Este documento está guardado cifrado en tu Archivos de la Bóveda ({kb} KB). Usa Exportar para guardarlo de nuevo en tu teléfono o compartirlo.',
    'export_button': 'Exportar',
    ...
    'encrypted_photo_sent_message': '🖼️ Foto cifrada enviada — ver en Archivos de la Bóveda Segura',
    'photo_chat_preview': '🖼️ Foto',
    ...
    'encrypted_file_sent_message': '📎 Archivo cifrado enviado — ver en Archivos de la Bóveda Segura',
    'file_chat_preview': '📎 Archivo',
    'encrypted_file_received': 'Archivo cifrado recibido — abre Archivos de la Bóveda Segura',
    'premium_required_title': 'SE REQUIERE PREMIUM',
    'premium_required_body': 'Guarda todas tus criptomonedas en una bóveda local de nivel militar. Envía fondos a cualquiera y recibe desde cualquier lugar, sin intermediarios y con privacidad absoluta.\n\nDesbloquear la Bóveda Cripto Segura requiere una suscripción Premium.',
    'monthly_button': 'MENSUAL',
    'yearly_button': 'ANUAL',
    'status_online': 'en línea',
    'status_offline': 'sin conexión',
    'status_waiting': 'esperando...',
    'security_alert_key_changed': 'ALERTA DE SEGURIDAD: Clave modificada. ¡Conexión bloqueada!',
    'default_username': 'Usuario',
    'node_destroyed': 'Nodo destruido',
    'call_handle_text': 'Llamada Cifrada',
    'call_accept_text': 'Contestar',
    'call_decline_text': 'Rechazar',
    'notif_title_new_message': 'Nuevo Mensaje',
    'notif_body_message': 'Has recibido un mensaje cifrado.',
    'notif_body_file': 'Has recibido un archivo cifrado.',
  },
  'FR': {
    'chats': 'Chats',
    'contacts': 'Contacts',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Coffre Crypto Sécurisé',
    ...
    'help_vault_files_q': 'Comment utiliser Fichiers du Coffre Sécurisé ?',
    ...
    'secure_crypto_vault_short': 'Coffre Crypto\nSécurisé',
    'secure_vault_files_short': 'Fichiers du\nCoffre Sécurisé',
    ...
    'crypto_vault_not_initialized': 'Coffre Crypto non initialisé sur cet appareil.',
    'invalid_crypto_vault_code': 'Code de Coffre Crypto invalide.',
    'crypto_vault_corrupted': 'Les données du Coffre Crypto sont corrompues (le code était correct, mais le fichier du coffre lui-même est endommagé).',
    'create_crypto_vault_code_title': 'CRÉER UN CODE COFFRE CRYPTO',
    'enter_crypto_vault_code_title': 'ENTRER LE CODE COFFRE CRYPTO',
    'crypto_code_desc_create': 'Ce code est distinct des codes de l\'app et de Fichiers du Coffre. Il protège un tout nouveau portefeuille non dépositaire que vous seul contrôlez.',
    'crypto_code_desc_enter': 'Entrez votre code Coffre Crypto pour accéder à votre portefeuille.',
    ...
    'set_crypto_vault_code_label': 'Définir le Code Coffre Crypto (pour CET appareil)',
    'crypto_vault_code_label': 'Code Coffre Crypto',
    ...
    'vault_files_not_initialized': 'Fichiers du Coffre n\'est pas initialisé sur cet appareil.',
    'invalid_vault_files_code': 'Code Fichiers du Coffre invalide.',
    'vault_files_corrupted': 'Les données de Fichiers du Coffre sont corrompues (le code était correct, mais le fichier du coffre lui-même est endommagé).',
    'create_vault_files_code_title': 'CRÉER LE CODE FICHIERS DU COFFRE',
    'enter_vault_files_code_title': 'SAISIR LE CODE FICHIERS DU COFFRE',
    ...
    'vault_files_code_desc_enter': 'Saisissez votre code Fichiers du Coffre pour consulter vos photos et documents chiffrés.',
    'set_vault_files_code_label': 'Définir le Code Fichiers du Coffre',
    'vault_files_code_label': 'Code Fichiers du Coffre',
    ...
    'document_label': 'Document',
    'document_stored_encrypted_desc': 'Ce document est stocké chiffré dans votre Fichiers du Coffre ({kb} Ko). Utilisez Exporter pour le sauvegarder sur votre téléphone ou le partager.',
    ...
    'encrypted_photo_sent_message': '🖼️ Photo chiffrée envoyée — voir dans Fichiers du Coffre Sécurisé',
    'photo_chat_preview': '🖼️ Photo',
    ...
    'encrypted_file_sent_message': '📎 Fichier chiffré envoyé — voir dans Fichiers du Coffre Sécurisé',
    'file_chat_preview': '📎 Fichier',
    'encrypted_file_received': 'Fichier chiffré reçu — ouvrez Fichiers du Coffre Sécurisé',
    'premium_required_title': 'PREMIUM REQUIS',
    'premium_required_body': 'Conservez toutes vos cryptomonnaies dans un coffre local de niveau militaire. Envoyez des fonds à n\'importe qui et recevez de n\'importe où, sans intermédiaire et avec une confidentialité absolue.\n\nDéverrouiller le Coffre Crypto Sécurisé nécessite un abonnement Premium.',
    'monthly_button': 'MENSUEL',
    'yearly_button': 'ANNUEL',
    'status_online': 'en ligne',
    'status_offline': 'hors ligne',
    'status_waiting': 'en attente...',
    'security_alert_key_changed': 'ALERTE DE SÉCURITÉ : Clé modifiée. Connexion bloquée !',
    'default_username': 'Utilisateur',
    'node_destroyed': 'Nœud détruit',
    'call_handle_text': 'Appel Chiffré',
    'call_accept_text': 'Répondre',
    'call_decline_text': 'Refuser',
    'notif_title_new_message': 'Nouveau Message',
    'notif_body_message': 'Vous avez reçu un message chiffré.',
    'notif_body_file': 'Vous avez reçu un fichier chiffré.',
  },
  'DE': {
    'chats': 'Chats',
    'contacts': 'Kontakte',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Sicherer Krypto-Tresor',
    ...
    'help_vault_files_q': 'Wie benutzt man Sichere Tresor-Dateien?',
    ...
    'secure_crypto_vault_short': 'Sicherer\nKrypto-Tresor',
    'secure_vault_files_short': 'Sichere\nTresor-Dateien',
    ...
    'crypto_vault_not_initialized': 'Krypto-Tresor auf diesem Gerät nicht initialisiert.',
    ...
    'crypto_code_desc_create': 'Dieser Code ist getrennt von den Codes der App und von Tresor-Dateien. Er schützt eine brandneue, nicht verwahrte Wallet, die nur Sie kontrollieren.',
    ...
    'vault_files_not_initialized': 'Tresor-Dateien ist auf diesem Gerät nicht initialisiert.',
    'invalid_vault_files_code': 'Ungültiger Tresor-Dateien-Code.',
    'vault_files_corrupted': 'Tresor-Dateien-Daten sind beschädigt (Code war korrekt, aber die Tresordatei selbst ist beschädigt).',
    'create_vault_files_code_title': 'TRESOR-DATEIEN-CODE ERSTELLEN',
    'enter_vault_files_code_title': 'TRESOR-DATEIEN-CODE EINGEBEN',
    ...
    'vault_files_code_desc_enter': 'Gib deinen Tresor-Dateien-Code ein, um deine verschlüsselten Fotos und Dokumente anzusehen.',
    'set_vault_files_code_label': 'Tresor-Dateien-Code festlegen',
    'vault_files_code_label': 'Tresor-Dateien-Code',
    ...
    'document_stored_encrypted_desc': 'Dieses Dokument ist verschlüsselt in deinem Tresor-Dateien gespeichert ({kb} KB). Nutze Exportieren, um es wieder auf dein Telefon zu speichern oder zu teilen.',
    ...
    'encrypted_photo_sent_message': '🖼️ Verschlüsseltes Foto gesendet — in Sichere Tresor-Dateien ansehen',
    'photo_chat_preview': '🖼️ Foto',
    ...
    'encrypted_file_sent_message': '📎 Verschlüsselte Datei gesendet — in Sichere Tresor-Dateien ansehen',
    'file_chat_preview': '📎 Datei',
    'encrypted_file_received': 'Verschlüsselte Datei empfangen — öffne Sichere Tresor-Dateien',
    'premium_required_title': 'PREMIUM ERFORDERLICH',
    'premium_required_body': 'Bewahre alle deine Kryptowährungen in einem lokalen Tresor auf Militärniveau auf. Sende Geld an jeden und empfange von überall – ohne Mittelsmänner und mit absoluter Privatsphäre.\n\nDas Freischalten des Sicheren Krypto-Tresors erfordert ein Premium-Abonnement.',
    'monthly_button': 'MONATLICH',
    'yearly_button': 'JÄHRLICH',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'wartet...',
    'security_alert_key_changed': 'SICHERHEITSWARNUNG: Schlüssel geändert. Verbindung blockiert!',
    'default_username': 'Benutzer',
    'node_destroyed': 'Knoten vernichtet',
    'call_handle_text': 'Verschlüsselter Anruf',
    'call_accept_text': 'Annehmen',
    'call_decline_text': 'Ablehnen',
    'notif_title_new_message': 'Neue Nachricht',
    'notif_body_message': 'Du hast eine verschlüsselte Nachricht erhalten.',
    'notif_body_file': 'Du hast eine verschlüsselte Datei erhalten.',
  },
  'RU': {
    'chats': 'Чаты',
    'contacts': 'Контакты',
    ...
    'section_premium': 'Padlock Премиум',
    'secure_crypto_vault_title': 'Защищённое крипто-хранилище',
    ...
    'help_vault_files_q': 'Как использовать Файлы защищённого хранилища?',
    ...
    'secure_crypto_vault_short': 'Защищённое\nкрипто-хранилище',
    'secure_vault_files_short': 'Файлы защищённого\nхранилища',
    ...
    'crypto_vault_not_initialized': 'крипто-хранилище не инициализирован на этом устройстве.',
    'invalid_crypto_vault_code': 'Неверный код крипто-хранилище.',
    'crypto_vault_corrupted': 'Данные крипто-хранилище повреждены (код был верным, но сам файл хранилища повреждён).',
    'create_crypto_vault_code_title': 'СОЗДАТЬ КОД КРИПТО-ХРАНИЛИЩЕ',
    'enter_crypto_vault_code_title': 'ВВЕСТИ КОД КРИПТО-ХРАНИЛИЩЕ',
    'crypto_code_desc_create': 'Этот код отделён от кодов приложения и файлы хранилища. Он защищает совершенно новый некастодиальный кошелёк, который контролируете только вы.',
    'crypto_code_desc_enter': 'Введите код крипто-хранилище для доступа к кошельку.',
    ...
    'set_crypto_vault_code_label': 'Установить код крипто-хранилище (для ЭТОГО устройства)',
    'crypto_vault_code_label': 'Код крипто-хранилище',
    ...
    'vault_files_not_initialized': 'файлы хранилища не инициализирован на этом устройстве.',
    'invalid_vault_files_code': 'Неверный код файлы хранилища.',
    'vault_files_corrupted': 'Данные файлы хранилища повреждены (код был верным, но сам файл хранилища повреждён).',
    'create_vault_files_code_title': 'СОЗДАТЬ КОД ФАЙЛЫ ХРАНИЛИЩА',
    'enter_vault_files_code_title': 'ВВЕДИТЕ КОД ФАЙЛЫ ХРАНИЛИЩА',
    ...
    'vault_files_code_desc_enter': 'Введите код файлы хранилища, чтобы просмотреть зашифрованные фото и документы.',
    'set_vault_files_code_label': 'Задать код файлы хранилища',
    'vault_files_code_label': 'Код файлы хранилища',
    ...
    'document_stored_encrypted_desc': 'Этот документ хранится зашифрованным в файлы хранилища ({kb} КБ). Используйте «Экспорт», чтобы сохранить его на телефон или поделиться им.',
    ...
    'encrypted_photo_sent_message': '🖼️ Зашифрованное фото отправлено — смотрите в Файлы защищённого хранилища',
    ...
    'encrypted_file_sent_message': '📎 Зашифрованный файл отправлен — смотрите в Файлы защищённого хранилища',
    'file_chat_preview': '📎 Файл',
    'encrypted_file_received': 'Получен зашифрованный файл — откройте Файлы защищённого хранилища',
    'premium_required_title': 'ТРЕБУЕТСЯ ПРЕМИУМ',
    'premium_required_body': 'Храните все свои криптовалюты в локальном хранилище военного уровня. Отправляйте средства кому угодно и получайте откуда угодно — без посредников и с абсолютной приватностью.\n\nДля разблокировки защищённого крипто-хранилища нужна подписка Премиум.',
    'monthly_button': 'В МЕСЯЦ',
    'yearly_button': 'В ГОД',
    'status_online': 'в сети',
    'status_offline': 'не в сети',
    'status_waiting': 'ожидание...',
    'security_alert_key_changed': 'ПРЕДУПРЕЖДЕНИЕ БЕЗОПАСНОСТИ: ключ изменён. Соединение заблокировано!',
    'default_username': 'Пользователь',
    'node_destroyed': 'Узел уничтожен',
    'call_handle_text': 'Зашифрованный звонок',
    'call_accept_text': 'Принять',
    'call_decline_text': 'Отклонить',
    'notif_title_new_message': 'Новое сообщение',
    'notif_body_message': 'Вы получили зашифрованное сообщение.',
    'notif_body_file': 'Вы получили зашифрованный файл.',
  },
  'UK': {
    'chats': 'Чати',
    'contacts': 'Контакти',
    ...
    'section_premium': 'Padlock Преміум',
    'secure_crypto_vault_title': 'Захищене крипто-сховище',
    ...
    'help_vault_files_q': 'Як використовувати Файли захищеного сховища?',
    ...
    'secure_crypto_vault_short': 'Захищене\nкрипто-сховище',
    'secure_vault_files_short': 'Файли захищеного\nсховища',
    ...
    'crypto_vault_not_initialized': 'крипто-сховище не ініціалізовано на цьому пристрої.',
    'invalid_crypto_vault_code': 'Невірний код крипто-сховище.',
    'crypto_vault_corrupted': 'Дані крипто-сховище пошкоджені (код був правильний, але сам файл сховища пошкоджено).',
    'create_crypto_vault_code_title': 'СТВОРИТИ КОД КРИПТО-СХОВИЩЕ',
    'enter_crypto_vault_code_title': 'ВВЕСТИ КОД КРИПТО-СХОВИЩЕ',
    'crypto_code_desc_create': 'Цей код відокремлений від кодів додатку та файли сховища. Він захищає абсолютно новий некастодіальний гаманець, який контролюєте лише ви.',
    'crypto_code_desc_enter': 'Введіть код крипто-сховище для доступу до гаманця.',
    ...
    'recovery_phrase_hint': 'слово1 слово2 слово3 ...',
    'set_crypto_vault_code_label': 'Встановити код крипто-сховище (для ЦЬОГО пристрою)',
    'crypto_vault_code_label': 'Код крипто-сховище',
    ...
    'vault_files_not_initialized': 'файли сховища не ініціалізовано на цьому пристрої.',
    'invalid_vault_files_code': 'Невірний код файли сховища.',
    'vault_files_corrupted': 'Дані файли сховища пошкоджені (код був правильним, але сам файл сховища пошкоджено).',
    'create_vault_files_code_title': 'СТВОРИТИ КОД ФАЙЛИ СХОВИЩА',
    'enter_vault_files_code_title': 'ВВЕДІТЬ КОД ФАЙЛИ СХОВИЩА',
    ...
    'vault_files_code_desc_enter': 'Введіть код файли сховища, щоб переглянути зашифровані фото й документи.',
    'set_vault_files_code_label': 'Встановити код файли сховища',
    'vault_files_code_label': 'Код файли сховища',
    ...
    'document_label': 'Документ',
    'document_stored_encrypted_desc': 'Цей документ зберігається зашифрованим у файли сховища ({kb} КБ). Використайте «Експорт», щоб зберегти його на телефон або поділитися ним.',
    ...
    'encrypted_photo_sent_message': '🖼️ Зашифроване фото надіслано — перегляньте в Файли захищеного сховища',
    'photo_chat_preview': '🖼️ Фото',
    'just_now': 'Щойно',
    ...
    'encrypted_file_sent_message': '📎 Зашифрований файл надіслано — перегляньте в Файли захищеного сховища',
    'file_chat_preview': '📎 Файл',
    'encrypted_file_received': 'Отримано зашифрований файл — відкрийте Файли захищеного сховища',
    'premium_required_title': 'ПОТРІБЕН ПРЕМІУМ',
    'premium_required_body': 'Зберігайте всі свої криптовалюти в локальному сховищі військового рівня. Надсилайте кошти будь-кому й отримуйте звідки завгодно — без посередників і з абсолютною приватністю.\n\nДля розблокування захищеного крипто-сховища потрібна підписка Преміум.',
    'monthly_button': 'НА МІСЯЦЬ',
    'yearly_button': 'НА РІК',
    'status_online': 'у мережі',
    'status_offline': 'не в мережі',
    'status_waiting': 'очікування...',
    'security_alert_key_changed': 'ПОПЕРЕДЖЕННЯ БЕЗПЕКИ: ключ змінено. З\'єднання заблоковано!',
    'default_username': 'Користувач',
    'node_destroyed': 'Вузол знищено',
    'call_handle_text': 'Зашифрований дзвінок',
    'call_accept_text': 'Прийняти',
    'call_decline_text': 'Відхилити',
    'notif_title_new_message': 'Нове повідомлення',
    'notif_body_message': 'Ви отримали зашифроване повідомлення.',
    'notif_body_file': 'Ви отримали зашифрований файл.',
  },
  'ZH': {
    'chats': '聊天',
    'contacts': '联系人',
    ...
    'section_premium': 'Padlock 高级版',
    'secure_crypto_vault_title': '安全加密货币保险库',
    'crypto_vault_desc': '为您的数字资产提供最高安全级别的存储。',
    'section_help_center': '帮助中心 / 使用方法',
    'help_vault_files_q': '如何使用 安全保险库文件？',
    ...
    'secure_crypto_vault_short': '安全加密货币保险库',
    'secure_vault_files_short': '安全保险库文件',
    ...
    'crypto_vault_not_initialized': '此设备上尚未初始化 加密货币保险库。',
    'invalid_crypto_vault_code': '加密货币保险库 密码无效。',
    'crypto_vault_corrupted': '加密货币保险库 数据已损坏（密码正确，但保险库文件本身已损坏）。',
    'create_crypto_vault_code_title': '创建 加密货币保险库 密码',
    'enter_crypto_vault_code_title': '输入 加密货币保险库 密码',
    'crypto_code_desc_create': '此密码与应用和 保险库文件 密码是分开的。它保护一个全新的、非托管的、只有您能控制的钱包。',
    'crypto_code_desc_enter': '输入您的 加密货币保险库 密码以访问您的钱包。',
    ...
    'set_crypto_vault_code_label': '设置 加密货币保险库 密码（用于此设备）',
    'crypto_vault_code_label': '加密货币保险库 密码',
    ...
    'vault_files_not_initialized': '此设备尚未初始化 保险库文件。',
    'invalid_vault_files_code': '保险库文件 代码无效。',
    'vault_files_corrupted': '保险库文件 数据已损坏(代码正确,但保险库文件本身已损坏)。',
    'create_vault_files_code_title': '创建 保险库文件 代码',
    'enter_vault_files_code_title': '输入 保险库文件 代码',
    'vault_files_code_desc_create': '此代码与您的应用解锁代码不同。知道应用代码的人如果没有此代码,将无法打开您的照片和文档。',
    'vault_files_code_desc_enter': '输入您的 保险库文件 代码以查看加密的照片和文档。',
    'set_vault_files_code_label': '设置 保险库文件 代码',
    'vault_files_code_label': '保险库文件 代码',
    ...
    'document_label': '文档',
    'document_stored_encrypted_desc': '此文档以加密方式保存在您的 保险库文件 中({kb} KB)。使用导出可将其保存回手机或分享。',
    'export_button': '导出',
    ...
    'encrypted_photo_sent_message': '🖼️ 已发送加密照片 — 在 安全保险库文件 中查看',
    ...
    'encrypted_file_sent_message': '📎 已发送加密文件 — 在 安全保险库文件 中查看',
    'file_chat_preview': '📎 文件',
    'encrypted_file_received': '收到加密文件 — 请打开 安全保险库文件',
    'premium_required_title': '需要高级版',
    'premium_required_body': '将您所有的加密货币存放在军用级本地保险库中。向任何人转账,从任何地方收款,没有中间人,绝对隐私。\n\n解锁安全加密货币保险库需要高级版订阅。',
    'monthly_button': '按月',
    'yearly_button': '按年',
    'status_online': '在线',
    'status_offline': '离线',
    'status_waiting': '等待中...',
    'security_alert_key_changed': '安全警报:密钥已更改。连接已被阻止!',
    'default_username': '用户',
    'node_destroyed': '节点已销毁',
    'call_handle_text': '加密通话',
    'call_accept_text': '接听',
    'call_decline_text': '拒绝',
    'notif_title_new_message': '新消息',
    'notif_body_message': '您收到了一条加密消息。',
    'notif_body_file': '您收到了一个加密文件。',
  },
  'KO': {
    'chats': '채팅',
    'contacts': '연락처',
    ...
    'section_premium': 'Padlock 프리미엄',
    'secure_crypto_vault_title': '보안 암호화폐 금고',
    'crypto_vault_desc': '디지털 자산을 위한 최고 보안 저장소.',
    'section_help_center': '도움말 센터 / 사용 방법',
    'help_vault_files_q': '보안 금고 파일 사용 방법은?',
    ...
    'secure_crypto_vault_short': '보안 암호화폐\n금고',
    'secure_vault_files_short': '보안 금고\n파일',
    ...
    'crypto_vault_not_initialized': '이 기기에서 암호화폐 금고가 초기화되지 않았습니다.',
    'invalid_crypto_vault_code': '암호화폐 금고 코드가 잘못되었습니다.',
    'crypto_vault_corrupted': '암호화폐 금고 데이터가 손상되었습니다(코드는 맞지만 보관함 파일 자체가 손상됨).',
    'create_crypto_vault_code_title': '암호화폐 금고 코드 생성',
    'enter_crypto_vault_code_title': '암호화폐 금고 코드 입력',
    'crypto_code_desc_create': '이 코드는 앱 및 금고 파일 코드와 별개입니다. 오직 사용자만 제어하는 완전히 새로운 비수탁형 지갑을 보호합니다.',
    'crypto_code_desc_enter': '지갑에 접근하려면 암호화폐 금고 코드를 입력하세요.',
    ...
    'set_crypto_vault_code_label': '암호화폐 금고 코드 설정 (이 기기용)',
    'crypto_vault_code_label': '암호화폐 금고 코드',
    ...
    'vault_files_not_initialized': '이 기기에서 금고 파일가 초기화되지 않았습니다.',
    'invalid_vault_files_code': '금고 파일 코드가 올바르지 않습니다.',
    'vault_files_corrupted': '금고 파일 데이터가 손상되었습니다(코드는 올바르지만 보관함 파일 자체가 손상됨).',
    'create_vault_files_code_title': '금고 파일 코드 생성',
    'enter_vault_files_code_title': '금고 파일 코드 입력',
    'vault_files_code_desc_create': '이 코드는 앱 잠금 해제 코드와 별개입니다. 앱 코드를 아는 사람도 이 코드 없이는 사진과 문서를 열 수 없습니다.',
    'vault_files_code_desc_enter': '암호화된 사진과 문서를 보려면 금고 파일 코드를 입력하세요.',
    'set_vault_files_code_label': '금고 파일 코드 설정',
    'vault_files_code_label': '금고 파일 코드',
    ...
    'document_label': '문서',
    'document_stored_encrypted_desc': '이 문서는 금고 파일에 암호화되어 저장되어 있습니다({kb} KB). 내보내기를 사용해 휴대폰에 다시 저장하거나 공유하세요.',
    'export_button': '내보내기',
    ...
    'encrypted_photo_sent_message': '🖼️ 암호화된 사진 전송됨 — 보안 금고 파일에서 확인하세요',
    ...
    'encrypted_file_sent_message': '📎 암호화된 파일 전송됨 — 보안 금고 파일에서 확인하세요',
    'file_chat_preview': '📎 파일',
    'encrypted_file_received': '암호화된 파일을 받았습니다 — 보안 금고 파일를 여세요',
    'premium_required_title': '프리미엄 필요',
    'premium_required_body': '모든 암호화폐를 군사급 로컬 금고에 보관하세요. 중개자 없이, 완벽한 프라이버시로 누구에게나 송금하고 어디서든 받을 수 있습니다.\n\n보안 암호화폐 금고를 잠금 해제하려면 프리미엄 구독이 필요합니다.',
    'monthly_button': '월간',
    'yearly_button': '연간',
    'status_online': '온라인',
    'status_offline': '오프라인',
    'status_waiting': '대기 중...',
    'security_alert_key_changed': '보안 경고: 키가 변경되었습니다. 연결이 차단되었습니다!',
    'default_username': '사용자',
    'node_destroyed': '노드 파기됨',
    'call_handle_text': '암호화된 통화',
    'call_accept_text': '수락',
    'call_decline_text': '거절',
    'notif_title_new_message': '새 메시지',
    'notif_body_message': '암호화된 메시지를 받았습니다.',
    'notif_body_file': '암호화된 파일을 받았습니다.',
  },
  'AR': {
    'chats': 'الدردشات',
    ...
    'section_premium': 'Padlock بريميوم',
    'secure_crypto_vault_title': 'خزنة العملات المشفرة الآمنة',
    'crypto_vault_desc': 'تخزين بأقصى درجات الأمان لأصولك الرقمية.',
    'section_help_center': 'مركز المساعدة / كيفية الاستخدام',
    'help_vault_files_q': 'كيف تستخدم ملفات الخزنة الآمنة؟',
    ...
    'secure_crypto_vault_short': 'خزنة العملات\nالمشفرة الآمنة',
    'secure_vault_files_short': 'ملفات الخزنة\nالآمنة',
    ...
    'crypto_vault_not_initialized': 'لم يتم تهيئة خزنة العملات المشفرة على هذا الجهاز.',
    'invalid_crypto_vault_code': 'رمز خزنة العملات المشفرة غير صالح.',
    'crypto_vault_corrupted': 'بيانات خزنة العملات المشفرة تالفة (الرمز كان صحيحًا، لكن ملف الخزنة نفسه تالف).',
    'create_crypto_vault_code_title': 'إنشاء رمز خزنة العملات المشفرة',
    'enter_crypto_vault_code_title': 'أدخل رمز خزنة العملات المشفرة',
    'crypto_code_desc_create': 'هذا الرمز منفصل عن رمز التطبيق ورموز ملفات الخزنة. إنه يحمي محفظة جديدة تمامًا غير وصائية تتحكم فيها أنت فقط.',
    'crypto_code_desc_enter': 'أدخل رمز خزنة العملات المشفرة للوصول إلى محفظتك.',
    ...
    'set_crypto_vault_code_label': 'تعيين رمز خزنة العملات المشفرة (لهذا الجهاز)',
    'crypto_vault_code_label': 'رمز خزنة العملات المشفرة',
    ...
    'vault_files_not_initialized': 'لم يتم تهيئة ملفات الخزنة على هذا الجهاز.',
    'invalid_vault_files_code': 'رمز ملفات الخزنة غير صالح.',
    'vault_files_corrupted': 'بيانات ملفات الخزنة تالفة (الرمز كان صحيحًا، لكن ملف الخزنة نفسه تالف).',
    'create_vault_files_code_title': 'إنشاء رمز ملفات الخزنة',
    'enter_vault_files_code_title': 'أدخل رمز ملفات الخزنة',
    ...
    'vault_files_code_desc_enter': 'أدخل رمز ملفات الخزنة لعرض صورك ومستنداتك المشفرة.',
    'set_vault_files_code_label': 'تعيين رمز ملفات الخزنة',
    'vault_files_code_label': 'رمز ملفات الخزنة',
    ...
    'document_stored_encrypted_desc': 'هذا المستند محفوظ مشفرًا في ملفات الخزنة ({kb} كيلوبايت). استخدم تصدير لحفظه مرة أخرى على هاتفك أو مشاركته.',
    'export_button': 'تصدير',
    ...
    'encrypted_photo_sent_message': '🖼️ تم إرسال صورة مشفرة — شاهدها في ملفات الخزنة الآمنة',
    ...
    'encrypted_file_sent_message': '📎 تم إرسال ملف مشفر — شاهده في ملفات الخزنة الآمنة',
    'file_chat_preview': '📎 ملف',
    'encrypted_file_received': 'تم استلام ملف مشفر — افتح ملفات الخزنة الآمنة',
    'premium_required_title': 'يلزم الاشتراك المميز',
    'premium_required_body': 'احتفظ بجميع عملاتك المشفرة في خزنة محلية بمستوى عسكري. أرسل الأموال إلى أي شخص واستلمها من أي مكان، بلا وسطاء وبخصوصية مطلقة.\n\nيتطلب فتح خزنة العملات المشفرة الآمنة اشتراكًا مميزًا.',
    'monthly_button': 'شهريًا',
    'yearly_button': 'سنويًا',
    'status_online': 'متصل',
    'status_offline': 'غير متصل',
    'status_waiting': 'قيد الانتظار...',
    'security_alert_key_changed': 'تنبيه أمني: تم تغيير المفتاح. تم حظر الاتصال!',
    'default_username': 'مستخدم',
    'node_destroyed': 'تم تدمير العقدة',
    'call_handle_text': 'مكالمة مشفرة',
    'call_accept_text': 'رد',
    'call_decline_text': 'رفض',
    'notif_title_new_message': 'رسالة جديدة',
    'notif_body_message': 'لقد تلقيت رسالة مشفرة.',
    'notif_body_file': 'لقد تلقيت ملفًا مشفرًا.',
  },
  'TR': {
    'chats': 'Sohbetler',
    'contacts': 'Kişiler',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Güvenli Kripto Kasası',
    ...
    'help_vault_files_q': 'Güvenli Kasa Dosyaları nasıl kullanılır?',
    ...
    'secure_crypto_vault_short': 'Güvenli Kripto\nKasası',
    'secure_vault_files_short': 'Güvenli Kasa\nDosyaları',
    ...
    'crypto_vault_not_initialized': 'Bu cihazda Kripto Kasası başlatılmadı.',
    'invalid_crypto_vault_code': 'Geçersiz Kripto Kasası kodu.',
    'crypto_vault_corrupted': 'Kripto Kasası verileri bozuk (kod doğruydu, ancak kasa dosyasının kendisi hasarlı).',
    'create_crypto_vault_code_title': 'KRIPTO KASASI KODU OLUŞTUR',
    'enter_crypto_vault_code_title': 'KRIPTO KASASI KODUNU GİRİN',
    'crypto_code_desc_create': 'Bu kod, uygulama ve Kasa Dosyaları kodlarından ayrıdır. Yalnızca sizin kontrol ettiğiniz yepyeni, saklayıcısız bir cüzdanı korur.',
    'crypto_code_desc_enter': 'Cüzdanınıza erişmek için Kripto Kasası kodunuzu girin.',
    ...
    'set_crypto_vault_code_label': 'Kripto Kasası Kodu Belirle (BU cihaz için)',
    'crypto_vault_code_label': 'Kripto Kasası Kodu',
    ...
    'vault_files_not_initialized': 'Kasa Dosyaları bu cihazda başlatılmadı.',
    'invalid_vault_files_code': 'Geçersiz Kasa Dosyaları kodu.',
    'vault_files_corrupted': 'Kasa Dosyaları verileri bozuk (kod doğruydu, ancak kasa dosyasının kendisi hasarlı).',
    'create_vault_files_code_title': 'KASA DOSYALARI KODU OLUŞTUR',
    'enter_vault_files_code_title': 'KASA DOSYALARI KODUNU GİRİN',
    ...
    'vault_files_code_desc_enter': 'Şifrelenmiş fotoğraflarınızı ve belgelerinizi görüntülemek için Kasa Dosyaları kodunuzu girin.',
    'set_vault_files_code_label': 'Kasa Dosyaları Kodunu Belirle',
    'vault_files_code_label': 'Kasa Dosyaları Kodu',
    ...
    'document_stored_encrypted_desc': 'Bu belge, Kasa Dosyalarınızda şifrelenmiş olarak saklanıyor ({kb} KB). Telefonunuza geri kaydetmek veya paylaşmak için Dışa Aktar\'ı kullanın.',
    ...
    'encrypted_photo_sent_message': '🖼️ Şifreli fotoğraf gönderildi — Güvenli Kasa Dosyaları\'nda görüntüleyin',
    ...
    'encrypted_file_sent_message': '📎 Şifreli dosya gönderildi — Güvenli Kasa Dosyaları\'nda görüntüleyin',
    'file_chat_preview': '📎 Dosya',
    'encrypted_file_received': 'Şifreli dosya alındı — Güvenli Kasa Dosyaları\'nı açın',
    'premium_required_title': 'PREMIUM GEREKLİ',
    'premium_required_body': 'Tüm kripto paralarınızı askeri düzeyde yerel bir kasada saklayın. Aracısız ve tam gizlilikle herkese para gönderin, her yerden alın.\n\nGüvenli Kripto Kasası\'nı açmak için Premium abonelik gerekir.',
    'monthly_button': 'AYLIK',
    'yearly_button': 'YILLIK',
    'status_online': 'çevrimiçi',
    'status_offline': 'çevrimdışı',
    'status_waiting': 'bekleniyor...',
    'security_alert_key_changed': 'GÜVENLİK UYARISI: Anahtar değişti. Bağlantı engellendi!',
    'default_username': 'Kullanıcı',
    'node_destroyed': 'Düğüm imha edildi',
    'call_handle_text': 'Şifreli Arama',
    'call_accept_text': 'Cevapla',
    'call_decline_text': 'Reddet',
    'notif_title_new_message': 'Yeni Mesaj',
    'notif_body_message': 'Şifreli bir mesaj aldınız.',
    'notif_body_file': 'Şifreli bir dosya aldınız.',
  },
  'IT': {
    'chats': 'Chat',
    'contacts': 'Contatti',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Cassaforte Crypto Sicura',
    ...
    'help_vault_files_q': 'Come usare File della Cassaforte Sicura?',
    ...
    'secure_crypto_vault_short': 'Cassaforte Crypto\nSicura',
    'secure_vault_files_short': 'File della\nCassaforte Sicura',
    ...
    'crypto_vault_not_initialized': 'Cassaforte Crypto non inizializzato su questo dispositivo.',
    'invalid_crypto_vault_code': 'Codice Cassaforte Crypto non valido.',
    'crypto_vault_corrupted': 'I dati del Cassaforte Crypto sono corrotti (il codice era corretto, ma il file del caveau stesso è danneggiato).',
    'create_crypto_vault_code_title': 'CREA CODICE CASSAFORTE CRYPTO',
    'enter_crypto_vault_code_title': 'INSERISCI CODICE CASSAFORTE CRYPTO',
    'crypto_code_desc_create': 'Questo codice è separato dai codici dell\'app e di File della Cassaforte. Protegge un portafoglio nuovo di zecca, non custodial, che controlli solo tu.',
    'crypto_code_desc_enter': 'Inserisci il tuo codice Cassaforte Crypto per accedere al tuo portafoglio.',
    ...
    'set_crypto_vault_code_label': 'Imposta Codice Cassaforte Crypto (per QUESTO dispositivo)',
    'crypto_vault_code_label': 'Codice Cassaforte Crypto',
    ...
    'vault_files_not_initialized': 'File della Cassaforte non è inizializzato su questo dispositivo.',
    'invalid_vault_files_code': 'Codice File della Cassaforte non valido.',
    'vault_files_corrupted': 'I dati di File della Cassaforte sono danneggiati (il codice era corretto, ma il file del caveau stesso è danneggiato).',
    'create_vault_files_code_title': 'CREA CODICE FILE DELLA CASSAFORTE',
    'enter_vault_files_code_title': 'INSERISCI CODICE FILE DELLA CASSAFORTE',
    ...
    'vault_files_code_desc_enter': 'Inserisci il tuo codice File della Cassaforte per visualizzare le tue foto e i tuoi documenti crittografati.',
    'set_vault_files_code_label': 'Imposta Codice File della Cassaforte',
    'vault_files_code_label': 'Codice File della Cassaforte',
    ...
    'document_label': 'Documento',
    'document_stored_encrypted_desc': 'Questo documento è salvato crittografato nel tuo File della Cassaforte ({kb} KB). Usa Esporta per salvarlo di nuovo sul telefono o condividerlo.',
    ...
    'encrypted_photo_sent_message': '🖼️ Foto crittografata inviata — visualizzala in File della Cassaforte Sicura',
    'photo_chat_preview': '🖼️ Foto',
    ...
    'encrypted_file_sent_message': '📎 File crittografato inviato — visualizzalo in File della Cassaforte Sicura',
    'file_chat_preview': '📎 File',
    'encrypted_file_received': 'File crittografato ricevuto — apri File della Cassaforte Sicura',
    'premium_required_title': 'RICHIESTO PREMIUM',
    'premium_required_body': 'Conserva tutte le tue criptovalute in una cassaforte locale di livello militare. Invia fondi a chiunque e ricevi da ovunque, senza intermediari e con privacy assoluta.\n\nSbloccare la Cassaforte Crypto Sicura richiede un abbonamento Premium.',
    'monthly_button': 'MENSILE',
    'yearly_button': 'ANNUALE',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'in attesa...',
    'security_alert_key_changed': 'AVVISO DI SICUREZZA: Chiave modificata. Connessione bloccata!',
    'default_username': 'Utente',
    'node_destroyed': 'Nodo distrutto',
    'call_handle_text': 'Chiamata Crittografata',
    'call_accept_text': 'Rispondi',
    'call_decline_text': 'Rifiuta',
    'notif_title_new_message': 'Nuovo Messaggio',
    'notif_body_message': 'Hai ricevuto un messaggio crittografato.',
    'notif_body_file': 'Hai ricevuto un file crittografato.',
  },
  'JA': {
    'chats': 'チャット',
    'contacts': '連絡先',
    ...
    'section_premium': 'Padlock プレミアム',
    'secure_crypto_vault_title': 'セキュア暗号資産ボールト',
    'crypto_vault_desc': 'デジタル資産のための最高セキュリティのストレージ。',
    'section_help_center': 'ヘルプセンター / 使い方',
    'help_vault_files_q': 'セキュアボールトファイルの使い方は？',
    ...
    'secure_crypto_vault_short': 'セキュア暗号資産ボールト',
    'secure_vault_files_short': 'セキュアボールトファイル',
    ...
    'crypto_vault_not_initialized': 'このデバイスで暗号資産ボールトが初期化されていません。',
    'invalid_crypto_vault_code': '暗号資産ボールトコードが無効です。',
    'crypto_vault_corrupted': '暗号資産ボールトのデータが破損しています（コードは正しいですが、保管庫ファイル自体が破損しています）。',
    'create_crypto_vault_code_title': '暗号資産ボールトコードを作成',
    'enter_crypto_vault_code_title': '暗号資産ボールトコードを入力',
    'crypto_code_desc_create': 'このコードはアプリやボールトファイルのコードとは別です。あなただけが管理する、まったく新しい非管理型ウォレットを保護します。',
    'crypto_code_desc_enter': 'ウォレットにアクセスするには暗号資産ボールトコードを入力してください。',
    ...
    'set_crypto_vault_code_label': '暗号資産ボールトコードを設定（このデバイス用）',
    'crypto_vault_code_label': '暗号資産ボールトコード',
    ...
    'vault_files_not_initialized': 'この端末では ボールトファイル が初期化されていません。',
    'invalid_vault_files_code': 'ボールトファイル コードが無効です。',
    'vault_files_corrupted': 'ボールトファイル のデータが破損しています(コードは正しいですが、金庫ファイル自体が破損しています)。',
    'create_vault_files_code_title': 'ボールトファイル コードを作成',
    'enter_vault_files_code_title': 'ボールトファイル コードを入力',
    'vault_files_code_desc_create': 'このコードはアプリのロック解除コードとは別のものです。アプリのコードを知っている人でも、このコードなしでは写真や書類を開くことができません。',
    'vault_files_code_desc_enter': '暗号化された写真や書類を表示するには、ボールトファイル コードを入力してください。',
    'set_vault_files_code_label': 'ボールトファイル コードを設定',
    'vault_files_code_label': 'ボールトファイル コード',
    ...
    'document_label': '書類',
    'document_stored_encrypted_desc': 'この書類は ボールトファイル に暗号化されて保存されています({kb} KB)。エクスポートを使って端末に保存し直したり、共有したりできます。',
    ...
    'encrypted_photo_sent_message': '🖼️ 暗号化された写真を送信しました — セキュアボールトファイル で確認してください',
    ...
    'encrypted_file_sent_message': '📎 暗号化されたファイルを送信しました — セキュアボールトファイル で確認してください',
    'file_chat_preview': '📎 ファイル',
    'encrypted_file_received': '暗号化されたファイルを受信しました — セキュアボールトファイル を開いてください',
    'premium_required_title': 'プレミアムが必要です',
    'premium_required_body': 'すべての暗号資産を軍用レベルのローカルボールトに保管できます。仲介者なし、完全なプライバシーで、誰にでも送金し、どこからでも受け取れます。\n\nセキュア暗号資産ボールトのロック解除にはプレミアム購読が必要です。',
    'monthly_button': '月額',
    'yearly_button': '年額',
    'status_online': 'オンライン',
    'status_offline': 'オフライン',
    'status_waiting': '待機中...',
    'security_alert_key_changed': 'セキュリティ警告:鍵が変更されました。接続がブロックされました!',
    'default_username': 'ユーザー',
    'node_destroyed': 'ノードを破棄しました',
    'call_handle_text': '暗号化通話',
    'call_accept_text': '応答',
    'call_decline_text': '拒否',
    'notif_title_new_message': '新しいメッセージ',
    'notif_body_message': '暗号化されたメッセージを受信しました。',
    'notif_body_file': '暗号化されたファイルを受信しました。',
  },
  'HI': {
    'chats': 'चैट',
    'contacts': 'संपर्क',
    ...
    'section_premium': 'Padlock प्रीमियम',
    'secure_crypto_vault_title': 'सुरक्षित क्रिप्टो वॉल्ट',
    'crypto_vault_desc': 'आपकी डिजिटल संपत्तियों के लिए अधिकतम सुरक्षा भंडारण।',
    'section_help_center': 'सहायता केंद्र / उपयोग कैसे करें',
    'help_vault_files_q': 'सुरक्षित वॉल्ट फ़ाइलें का उपयोग कैसे करें?',
    ...
    'secure_crypto_vault_short': 'सुरक्षित क्रिप्टो\nवॉल्ट',
    'secure_vault_files_short': 'सुरक्षित वॉल्ट\nफ़ाइलें',
    ...
    'crypto_vault_not_initialized': 'इस डिवाइस पर क्रिप्टो वॉल्ट प्रारंभ नहीं किया गया है।',
    'invalid_crypto_vault_code': 'अमान्य क्रिप्टो वॉल्ट कोड।',
    'crypto_vault_corrupted': 'क्रिप्टो वॉल्ट डेटा दूषित है (कोड सही था, लेकिन वॉल्ट फ़ाइल स्वयं क्षतिग्रस्त है)।',
    'create_crypto_vault_code_title': 'क्रिप्टो वॉल्ट कोड बनाएं',
    'enter_crypto_vault_code_title': 'क्रिप्टो वॉल्ट कोड दर्ज करें',
    'crypto_code_desc_create': 'यह कोड आपके ऐप और वॉल्ट फ़ाइलें कोड से अलग है। यह एक बिल्कुल नए, गैर-कस्टोडियल वॉलेट की रक्षा करता है जिसे केवल आप नियंत्रित करते हैं।',
    'crypto_code_desc_enter': 'अपने वॉलेट तक पहुंचने के लिए अपना क्रिप्टो वॉल्ट कोड दर्ज करें।',
    ...
    'set_crypto_vault_code_label': 'क्रिप्टो वॉल्ट कोड सेट करें (इस डिवाइस के लिए)',
    'crypto_vault_code_label': 'क्रिप्टो वॉल्ट कोड',
    ...
    'vault_files_not_initialized': 'इस डिवाइस पर वॉल्ट फ़ाइलें प्रारंभ नहीं किया गया है।',
    'invalid_vault_files_code': 'अमान्य वॉल्ट फ़ाइलें कोड।',
    'vault_files_corrupted': 'वॉल्ट फ़ाइलें डेटा दूषित है (कोड सही था, लेकिन वॉल्ट फ़ाइल स्वयं क्षतिग्रस्त है)।',
    'create_vault_files_code_title': 'वॉल्ट फ़ाइलें कोड बनाएं',
    'enter_vault_files_code_title': 'वॉल्ट फ़ाइलें कोड दर्ज करें',
    ...
    'vault_files_code_desc_enter': 'अपनी एन्क्रिप्टेड तस्वीरें और दस्तावेज़ देखने के लिए अपना वॉल्ट फ़ाइलें कोड दर्ज करें।',
    'set_vault_files_code_label': 'वॉल्ट फ़ाइलें कोड सेट करें',
    'vault_files_code_label': 'वॉल्ट फ़ाइलें कोड',
    ...
    'document_stored_encrypted_desc': 'यह दस्तावेज़ आपके वॉल्ट फ़ाइलें में एन्क्रिप्टेड रूप से संग्रहीत है ({kb} KB)। इसे फिर से फ़ोन में सहेजने या साझा करने के लिए एक्सपोर्ट का उपयोग करें।',
    ...
    'encrypted_photo_sent_message': '🖼️ एन्क्रिप्टेड फ़ोटो भेजी गई — सुरक्षित वॉल्ट फ़ाइलें में देखें',
    ...
    'encrypted_file_sent_message': '📎 एन्क्रिप्टेड फ़ाइल भेजी गई — सुरक्षित वॉल्ट फ़ाइलें में देखें',
    'file_chat_preview': '📎 फ़ाइल',
    'encrypted_file_received': 'एन्क्रिप्टेड फ़ाइल प्राप्त हुई — सुरक्षित वॉल्ट फ़ाइलें खोलें',
    'premium_required_title': 'प्रीमियम आवश्यक',
    'premium_required_body': 'अपनी सभी क्रिप्टोकरेंसी को सैन्य-स्तर के स्थानीय वॉल्ट में रखें। बिना बिचौलियों और पूर्ण गोपनीयता के किसी को भी धन भेजें और कहीं से भी प्राप्त करें।\n\nसुरक्षित क्रिप्टो वॉल्ट अनलॉक करने के लिए प्रीमियम सदस्यता आवश्यक है।',
    'monthly_button': 'मासिक',
    'yearly_button': 'वार्षिक',
    'status_online': 'ऑनलाइन',
    'status_offline': 'ऑफ़लाइन',
    'status_waiting': 'प्रतीक्षा में...',
    'security_alert_key_changed': 'सुरक्षा चेतावनी: कुंजी बदल गई। कनेक्शन अवरुद्ध!',
    'default_username': 'उपयोगकर्ता',
    'node_destroyed': 'नोड नष्ट किया गया',
    'call_handle_text': 'एन्क्रिप्टेड कॉल',
    'call_accept_text': 'उत्तर दें',
    'call_decline_text': 'अस्वीकार करें',
    'notif_title_new_message': 'नया संदेश',
    'notif_body_message': 'आपको एक एन्क्रिप्टेड संदेश मिला है।',
    'notif_body_file': 'आपको एक एन्क्रिप्टेड फ़ाइल मिली है।',
  },
  'NL': {
    'chats': 'Chats',
    'contacts': 'Contacten',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Beveiligde Cryptokluis',
    ...
    'help_vault_files_q': 'Hoe gebruik ik Beveiligde Kluisbestanden?',
    ...
    'secure_crypto_vault_short': 'Beveiligde\nCryptokluis',
    'secure_vault_files_short': 'Beveiligde\nKluisbestanden',
    'privacy_id_label': 'Privacy-ID',
    ...
    'crypto_vault_not_initialized': 'Cryptokluis is niet geïnitialiseerd op dit apparaat.',
    'invalid_crypto_vault_code': 'Ongeldige Cryptokluis-code.',
    'crypto_vault_corrupted': 'Cryptokluis-gegevens zijn beschadigd (code was correct, maar het kluisbestand zelf is beschadigd).',
    'create_crypto_vault_code_title': 'MAAK CRYPTOKLUIS-CODE',
    'enter_crypto_vault_code_title': 'VOER CRYPTOKLUIS-CODE IN',
    'crypto_code_desc_create': 'Deze code is anders dan die van je app en Kluisbestanden. Het beschermt een volledig nieuwe, non-custodiale wallet die alleen jij beheert.',
    'crypto_code_desc_enter': 'Voer je Cryptokluis-code in om toegang te krijgen tot je wallet.',
    ...
    'set_crypto_vault_code_label': 'Stel Cryptokluis-code in (voor dit apparaat)',
    'crypto_vault_code_label': 'Cryptokluis-code',
    ...
    'vault_files_not_initialized': 'Kluisbestanden is niet geïnitialiseerd op dit apparaat.',
    'invalid_vault_files_code': 'Ongeldige Kluisbestanden-code.',
    'vault_files_corrupted': 'Kluisbestanden-gegevens zijn beschadigd (code was correct, maar het kluisbestand zelf is beschadigd).',
    'create_vault_files_code_title': 'MAAK KLUISBESTANDEN-CODE',
    'enter_vault_files_code_title': 'VOER KLUISBESTANDEN-CODE IN',
    ...
    'vault_files_code_desc_enter': 'Voer je Kluisbestanden-code in om je versleutelde foto\'s en documenten te bekijken.',
    'set_vault_files_code_label': 'Stel Kluisbestanden-code in',
    'vault_files_code_label': 'Kluisbestanden-code',
    ...
    'document_label': 'Document',
    'document_stored_encrypted_desc': 'Dit document is versleuteld opgeslagen in je Kluisbestanden ({kb} KB). Gebruik Exporteren om het weer op je telefoon op te slaan of te delen.',
    ...
    'encrypted_photo_sent_message': '🖼️ Versleutelde foto verzonden — bekijk in Beveiligde Kluisbestanden',
    'photo_chat_preview': '🖼️ Foto',
    'just_now': 'Zojuist',
    ...
    'encrypted_file_sent_message': '📎 Versleuteld bestand verzonden — bekijk in Beveiligde Kluisbestanden',
    'file_chat_preview': '📎 Bestand',
    'encrypted_file_received': 'Versleuteld bestand ontvangen — open Beveiligde Kluisbestanden',
    'premium_required_title': 'PREMIUM VEREIST',
    'premium_required_body': 'Bewaar al je cryptovaluta in een lokale kluis van militair niveau. Stuur geld naar iedereen en ontvang van overal, zonder tussenpersonen en met absolute privacy.\n\nHet ontgrendelen van de Beveiligde Cryptokluis vereist een Premium-abonnement.',
    'monthly_button': 'MAANDELIJKS',
    'yearly_button': 'JAARLIJKS',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'wachten...',
    'security_alert_key_changed': 'BEVEILIGINGSWAARSCHUWING: Sleutel gewijzigd. Verbinding geblokkeerd!',
    'default_username': 'Gebruiker',
    'node_destroyed': 'Knooppunt vernietigd',
    'call_handle_text': 'Versleuteld Gesprek',
    'call_accept_text': 'Opnemen',
    'call_decline_text': 'Weigeren',
    'notif_title_new_message': 'Nieuw Bericht',
    'notif_body_message': 'Je hebt een versleuteld bericht ontvangen.',
    'notif_body_file': 'Je hebt een versleuteld bestand ontvangen.',
  },
  'PL': {
    'chats': 'Czaty',
    'contacts': 'Kontakty',
    ...
    'section_premium': 'Padlock Premium',
    'secure_crypto_vault_title': 'Bezpieczny Skarbiec Kryptowalut',
    ...
    'help_vault_files_q': 'Jak korzystać z Pliki Bezpiecznego Skarbca?',
    ...
    'secure_crypto_vault_short': 'Bezpieczny Skarbiec\nKryptowalut',
    'secure_vault_files_short': 'Pliki Bezpiecznego\nSkarbca',
    ...
    'crypto_vault_not_initialized': 'Skarbiec Kryptowalut nie został zainicjowany na tym urządzeniu.',
    'invalid_crypto_vault_code': 'Nieprawidłowy kod Skarbiec Kryptowalut.',
    'crypto_vault_corrupted': 'Dane Skarbiec Kryptowalut są uszkodzone (kod był poprawny, ale sam plik skarbca jest uszkodzony).',
    'create_crypto_vault_code_title': 'UTWÓRZ KOD SKARBIEC KRYPTOWALUT',
    'enter_crypto_vault_code_title': 'WPROWADŹ KOD SKARBIEC KRYPTOWALUT',
    'crypto_code_desc_create': 'Ten kod różni się od kodu aplikacji i Pliki Skarbca. Chroni zupełnie nowy, niekustodialny portfel, który kontrolujesz tylko Ty.',
    'crypto_code_desc_enter': 'Wprowadź swój kod Skarbiec Kryptowalut, aby uzyskać dostęp do portfela.',
    ...
    'set_crypto_vault_code_label': 'Ustaw kod Skarbiec Kryptowalut (dla tego urządzenia)',
    'crypto_vault_code_label': 'Kod Skarbiec Kryptowalut',
    ...
    'vault_files_not_initialized': 'Pliki Skarbca nie został zainicjowany na tym urządzeniu.',
    'invalid_vault_files_code': 'Nieprawidłowy kod Pliki Skarbca.',
    'vault_files_corrupted': 'Dane Pliki Skarbca są uszkodzone (kod był poprawny, ale sam plik skarbca jest uszkodzony).',
    'create_vault_files_code_title': 'UTWÓRZ KOD PLIKI SKARBCA',
    'enter_vault_files_code_title': 'WPROWADŹ KOD PLIKI SKARBCA',
    ...
    'vault_files_code_desc_enter': 'Wprowadź kod Pliki Skarbca, aby wyświetlić zaszyfrowane zdjęcia i dokumenty.',
    'set_vault_files_code_label': 'Ustaw kod Pliki Skarbca',
    'vault_files_code_label': 'Kod Pliki Skarbca',
    ...
    'document_label': 'Dokument',
    'document_stored_encrypted_desc': 'Ten dokument jest przechowywany zaszyfrowany w Pliki Skarbca ({kb} KB). Użyj Eksportuj, aby zapisać go z powrotem na telefonie lub udostępnić.',
    ...
    'encrypted_photo_sent_message': '🖼️ Wysłano zaszyfrowane zdjęcie — zobacz w Pliki Bezpiecznego Skarbca',
    ...
    'encrypted_file_sent_message': '📎 Wysłano zaszyfrowany plik — zobacz w Pliki Bezpiecznego Skarbca',
    'file_chat_preview': '📎 Plik',
    'encrypted_file_received': 'Otrzymano zaszyfrowany plik — otwórz Pliki Bezpiecznego Skarbca',
    'premium_required_title': 'WYMAGANY PREMIUM',
    'premium_required_body': 'Przechowuj wszystkie swoje kryptowaluty w lokalnym skarbcu klasy wojskowej. Wysyłaj środki komukolwiek i odbieraj z dowolnego miejsca — bez pośredników i z absolutną prywatnością.\n\nOdblokowanie Bezpiecznego Skarbca Kryptowalut wymaga subskrypcji Premium.',
    'monthly_button': 'MIESIĘCZNIE',
    'yearly_button': 'ROCZNIE',
    'status_online': 'online',
    'status_offline': 'offline',
    'status_waiting': 'oczekiwanie...',
    'security_alert_key_changed': 'ALERT BEZPIECZEŃSTWA: Klucz zmieniony. Połączenie zablokowane!',
    'default_username': 'Użytkownik',
    'node_destroyed': 'Węzeł zniszczony',
    'call_handle_text': 'Szyfrowane Połączenie',
    'call_accept_text': 'Odbierz',
    'call_decline_text': 'Odrzuć',
    'notif_title_new_message': 'Nowa Wiadomość',
    'notif_body_message': 'Otrzymano zaszyfrowaną wiadomość.',
    'notif_body_file': 'Otrzymano zaszyfrowany plik.',
  },
};
...
@override
  void initState() {
    ...
    _username = (t[_currentLang] ?? t['EN']!)['loading_text']!;
      ...
      _hubSubscription = PadlockNetwork.messageHub.stream.listen((message) async {
        ...
        for (var chat in _chats) {
          if (chat['id'] == peerId) {
            ...
            chat['msg'] = (t[_currentLang] ?? t['EN']!)['node_destroyed']!;
          }
        }
...
if (chaveTrancada == null) {
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    SnackBar(
      content: Text((t[_currentLang] ?? t['EN']!)['security_alert_key_changed']!),
...
final int msgTimestamp = data['timestamp'] ?? 0;
    if (msgTimestamp > _lastNotifiedTimestamp) {
      ...
      showNotification((t[_currentLang] ?? t['EN']!)['notif_title_new_message']!, (t[_currentLang] ?? t['EN']!)['notif_body_message']!);
    }
      ...
      setState(() {
        ...
        if (chatIdx != -1) {
            ...
            Hive.box('padlock_vault').put('chats', jsonEncode(_chats));
            showNotification((t[_currentLang] ?? t['EN']!)['notif_title_new_message']!, (t[_currentLang] ?? t['EN']!)['notif_body_file']!);
          }
...
@override
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.paused) {
      ...
      _pausedAt = null;
      // Se houve uma chamada durante o tempo em que a app esteve em segundo
      // plano (ex: chamada longa com o ecrã apagado), esse tempo NÃO conta -
      // senão, ao desligar a chamada e voltar à app, era logo expulso.
      final callDuringPause = PadlockNetwork.callLastActive != null &&
          pausedAt != null &&
          PadlockNetwork.callLastActive!.isAfter(pausedAt);
      if (pausedAt != null &&
          !callDuringPause &&
   ...
   Future<void> _loadUsername() async {
    ...
    setState(() {
      _username = name ?? (t[_currentLang] ?? t['EN']!)['default_username']!;
    });
  }
            ...
            if (decoded['type'] == 'wipe_chat') {
              if (mounted) {
                setState(() {
                  ...
                  widget.chatData['msg'] = widget.local['node_destroyed']!;
                });
                widget.onUpdate();
              }
              return;
...
Widget build(BuildContext context) {
  ...
  builder: (context, status, child) {
    ...
    final statusText = status == 'Online'
        ? local['status_online']!
        : (status == 'Aguardar...' ? local['status_waiting']! : local['status_offline']!);
    return Text(
      statusText,
      style: TextStyle(
        color: statusColor, 
...
bool _isRemoteSet = false;
  ...
  void initState() {
    ...
    PadlockNetwork.callLastActive = DateTime.now();
    PadlockNetwork.emChamada = true;
    WakelockPlus.enable();
...
void showPremiumRequiredDialog(BuildContext context) {
  final premiumLang = context.findAncestorStateOfType<_PadlockAppState>()?._currentLanguage ?? 'EN';
  final local = t[premiumLang] ?? t['EN']!;
  showDialog(
    context: context,
    builder: (ctx) => AlertDialog(
      backgroundColor: const Color(0xFF151515),
      ...
      title: GestureDetector(
        ...
        child: Row(
          children: [
            const Text('💎', style: TextStyle(fontSize: 22)),
            const SizedBox(width: 10),
            Flexible(child: Text(local['premium_required_title']!, style: const TextStyle(color: Colors.lightBlueAccent, fontWeight: FontWeight.bold, fontSize: 14))),
          ],
        ),
      ),
      content: Text(
        local['premium_required_body']!,
        style: const TextStyle(color: Colors.white70, height: 1.4, fontSize: 13),
      ),
      actions: [
        TextButton(
          onPressed: () => Navigator.pop(ctx),
          child: Text(local['close_button']!.toUpperCase(), style: const TextStyle(color: Colors.grey)),
        ),
        TextButton(
          onPressed: () async {
            Navigator.pop(ctx);
          ...
          child: Text(local['monthly_button']!, style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
        ),
        TextButton(
          onPressed: () async {
            Navigator.pop(ctx);
          ...
          child: Text(local['yearly_button']!, style: const TextStyle(color: Colors.amber, fontWeight: FontWeight.bold)),
        ),
      ],
    ),
  );
